...
/**
 * Get payment method display name based on order data
 * Requirements: 8.1, 8.3
 */
function getPaymentMethod(order: any): string {
  // Check marketplace_platform field first
  if (order.marketplace_platform) {
    // Normalize the platform name (handle case variations)
    const platform = order.marketplace_platform.toLowerCase();
    if (platform === 'shopee') return 'Shopee';
    if (platform === 'tokopedia') return 'Tokopedia';
    // Return as-is for other platforms (e.g., PadiUMKM)
    return order.marketplace_platform;
  }
  
  // Default to Midtrans for orders without marketplace_platform
  return 'Midtrans';
}
      ...
      {hasError ? (
        <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-6 text-center">
          ...
          <p className="text-muted-foreground mb-4">
            Data pesanan sementara tidak tersedia. Silakan coba lagi nanti.
          </p>
          <Button
                    ...
                    <div className="relative z-10">
                      <div className="flex flex-col md:flex-row md:items-start justify-between mb-6 gap-4">
                        <div>
                          ...
                          {/* Payment method display - Requirements 8.1, 8.3 */}
                          <p className="text-sm text-muted-foreground">
                            Metode Pembayaran:{' '}
